...
class ExcavatorLSTM(nn.Module):
    # (保持不变)
    def __init__(self, input_size=12, hidden_size=128, output_size=8, num_layers=2, dropout=0.1):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_prefix', type=str, default='excavator_lstm_raw_angle_custom_loss', help='Prefix for saving model and scaler files')
    ...
    try:
        qpos = qpos_all[:, qpos_indices]; qvel = qvel_all[:, qvel_indices]
    ...
    scaler_filename = f"{args.output_prefix}_scaler.joblib"; joblib.dump(scaler, scaler_filename); print(f"Input feature scaler saved to {scaler_filename}")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0: print(f'Epoch [{epoch+1}/{args.epochs}], Loss: {avg_loss:.6f}')
        # Simple early stopping based on training loss improvement
        if avg_loss < best_loss: best_loss = avg_loss; epochs_no_improve = 0
    ...
    # --- 8. Save Model ---
    # (保持不变)
    model_filename = f"{args.output_prefix}_model.pth"; torch.save(model.state_dict(), model_filename); print(f"Model saved to {model_filename}")

    # --- 9. Evaluation ---
    print("Evaluating model...")
    model.eval()
    test_loss_single_step = 0
    # --- 新增: 存储单步预测结果 ---
    all_single_step_preds = []
    all_single_step_targets = []
    # --- 结束新增 ---
    with torch.no_grad():
        for batch_x, batch_y in test_loader:
            outputs, _ = model(batch_x)
            loss = criterion(outputs, batch_y)
            test_loss_single_step += loss.item()
            # --- 新增: 收集单步预测 ---
            all_single_step_preds.append(outputs.cpu().numpy())
            all_single_step_targets.append(batch_y.cpu().numpy())
            # --- 结束新增 ---
    ...
    start_index = 0 # Choose a starting point in the test sequences
    if len(X_test_seq) <= start_index:
        print("Not enough test sequences for evaluation.")
    else:
        # --- Rollout Prediction ---
        ...
        last_predicted_qpos_rollout = y_test_seq[start_index, :4].flatten()
        last_predicted_qvel_rollout = y_test_seq[start_index, 4:].flatten()
        predictions_rollout_pos.append(last_predicted_qpos_rollout)
        predictions_rollout_vel.append(last_predicted_qvel_rollout)
        ...
        with torch.no_grad():
            for i in range(1, rollout_steps):
                ...
                predictions_rollout_pos.append(next_qpos_pred)
                predictions_rollout_vel.append(next_qvel_pred)
        ...
        predictions_single_step_pos = []
        predictions_single_step_vel = []
        with torch.no_grad():
             # Iterate through the sequences used in the rollout ground truth
             for i in range(start_index, start_index + len(predictions_rollout_pos)):
                  if i >= len(X_test_seq): break # Ensure index is valid
                  ...
                  predictions_single_step_pos.append(pred_output[:4])
                  predictions_single_step_vel.append(pred_output[4:])

        predictions_single_step_pos = np.array(predictions_single_step_pos)
        predictions_single_step_vel = np.array(predictions_single_step_vel)

        # --- Ground Truth for the segment ---
        ...
        plt.title('Training Loss over Epochs'); plt.xlabel('Epoch'); plt.ylabel('Custom Loss')
        plt.legend(); plt.grid(True); plt.savefig(f"{args.output_prefix}_training_loss.png"); plt.close()
        ...
        for i in range(num_joints_to_plot):
            ...
            # --- 结束新增 ---
            axes[i, 0].set_ylabel(f'{joint_names[i]} Pos (deg)')
            ...
            # --- 结束新增 ---
            axes[i, 1].set_ylabel(f'{joint_names[i]} Vel (rad/s)')
            axes[i, 1].legend(); axes[i, 1].grid(True)

        axes[-1, 0].set_xlabel('Time (s)')
        axes[-1, 1].set_xlabel('Time (s)')
        fig.suptitle('Single-Step vs Multi-step Rollout Prediction vs Ground Truth (LSTM)') # Updated title
        plt.tight_layout(rect=[0, 0.03, 1, 0.96])
        plt.savefig(f"{args.output_prefix}_rollout_vs_single_step.png") # Updated filename
        print(f"Comparison plot saved to {args.output_prefix}_rollout_vs_single_step.png")
        plt.close(fig)

        # --- 12. Calculate & Print Errors for Both Modes ---
        # Rollout Errors
        rollout_qpos_error = np.zeros_like(predictions_rollout_pos)
        rollout_qpos_error[:, non_periodic_idx] = predictions_rollout_pos[:, non_periodic_idx] - ground_truth_rollout_pos[:, non_periodic_idx]
        rollout_qpos_error[:, periodic_idx] = angle_difference(predictions_rollout_pos[:, periodic_idx], ground_truth_rollout_pos[:, periodic_idx])
        ...
        print("\n--- Rollout Prediction Errors ---")
        print(f"Qpos MAE: {rollout_qpos_mae:.6f} rad"); print(f"Qvel MAE: {rollout_qvel_mae:.6f} rad/s")
        print(f"Qpos RMSE: {rollout_qpos_rmse:.6f} rad"); print(f"Qvel RMSE: {rollout_qvel_rmse:.6f} rad/s")

        # Single-Step Errors
        single_step_qpos_error = np.zeros_like(predictions_single_step_pos)
        single_step_qpos_error[:, non_periodic_idx] = predictions_single_step_pos[:, non_periodic_idx] - ground_truth_rollout_pos[:, non_periodic_idx] # Use same ground truth segment
        single_step_qpos_error[:, periodic_idx] = angle_difference(predictions_single_step_pos[:, periodic_idx], ground_truth_rollout_pos[:, periodic_idx])
        single_step_qvel_error = predictions_single_step_vel - ground_truth_rollout_vel
        single_step_qpos_mae = np.mean(np.abs(single_step_qpos_error))
        single_step_qvel_mae = np.mean(np.abs(single_step_qvel_error))
        single_step_qpos_rmse = np.sqrt(np.mean(single_step_qpos_error**2))
        single_step_qvel_rmse = np.sqrt(np.mean(single_step_qvel_error**2))
        print("\n--- Single-Step Prediction Errors (on rollout segment) ---")
        print(f"Qpos MAE: {single_step_qpos_mae:.6f} rad"); print(f"Qvel MAE: {single_step_qvel_mae:.6f} rad/s")
        print(f"Qpos RMSE: {single_step_qpos_rmse:.6f} rad"); print(f"Qvel RMSE: {single_step_qvel_rmse:.6f} rad/s")
