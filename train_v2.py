...
def create_sequences(input_features, output_labels, seq_length):
    xs, ys = [], []
    if len(input_features) < seq_length: # Need at least seq_length points for input
        return np.empty((0, seq_length, input_features.shape[1])), np.empty((0, output_labels.shape[1]))
    # Labels correspond to the state AFTER the sequence
    num_sequences = len(input_features) - seq_length
    if num_sequences < 0: # Should not happen with above check, but safety
         return np.empty((0, seq_length, input_features.shape[1])), np.empty((0, output_labels.shape[1]))

    for i in range(num_sequences + 1): # Iterate to create all possible sequences
        start_idx = i
        end_idx = i + seq_length
        label_idx = end_idx -1 # Label corresponds to state at end_idx (time t+seq_length)

        # Ensure label index is within bounds of output_labels
        if label_idx < len(output_labels):
            x = input_features[start_idx:end_idx]
            y = output_labels[label_idx]
            xs.append(x)
            ys.append(y)
        # else: # Optional: Print warning if label index goes out of bounds
            # print(f"Warning: Label index {label_idx} out of bounds for output_labels length {len(output_labels)}.")
            # break # Stop if labels run out
...
if __name__ == "__main__":
    ...
    # Store original qpos/qvel labels for later analysis
    original_output_qpos = qpos_labels_all.copy()
    original_output_qvel = qvel_labels_all.copy()
    ...
    X_train_val, X_test, y_train_val_unscaled_vel, y_test_unscaled_vel, time_train_val, time_test = train_test_split(
        input_features, output_labels_unscaled_vel, time_labels, test_size=args.test_split, random_state=args.seed, shuffle=False)
    ...
    y_test_original = y_test_unscaled_vel.copy()


    # --- 5. Sequence Creation ---
    # ... (同上一版本, 修正 create_sequences 调用和时间对齐) ...
    ...
    # Align time axes with the END of the sequences (which corresponds to the label time)
    ...
    # Trim sequences to match the minimum length available across X, y, and time
    train_seq_len = min(len(X_train_seq), len(y_train_seq), len(time_train_seq_end))
    val_seq_len = min(len(X_val_seq), len(y_val_seq), len(time_val_seq_end))
    test_seq_len = min(len(X_test_seq), len(y_test_seq), len(time_test_seq_end))
    ...
    time_test_seq_end = time_test_seq_end[:test_seq_len] # This time corresponds to the labels y_test_seq
    ...
    if test_loader is None:
        print("Skipping test set evaluation as test set is empty.")
        all_test_preds_processed = np.empty((0, output_size))
        test_seq_len = 0 # Ensure test_seq_len is 0 if no test loader
    else:
        print("Evaluating model on Test Set (Single Step)...")
        model.eval(); test_loss_final = 0; all_test_preds_processed_list = []
        with torch.no_grad():
        ...
        avg_test_loss_final = test_loss_final / max(1, len(test_loader))
        ...
        # test_seq_len is already defined from sequence creation stage
    ...
    if test_seq_len > 0 and len(all_test_preds_processed) == test_seq_len:
        ...
        # ... (Deconstruction and recovery code identical to previous version) ...
        ...
        gt_align_start_idx = len(X_train) + len(X_val) + args.seq_length - 1 # Index in original_output arrays
        gt_align_end_idx = gt_align_start_idx + test_seq_len
        if gt_align_end_idx > len(original_output_qpos): # Adjust if index goes out of bounds
             print(f"Warning: Ground truth alignment index {gt_align_end_idx} exceeds data length {len(original_output_qpos)}. Trimming test sequence length.")
             gt_align_end_idx = len(original_output_qpos)
             test_seq_len = gt_align_end_idx - gt_align_start_idx
             # Trim predictions to match available ground truth
             single_step_pred_qpos_orig = single_step_pred_qpos_orig[:test_seq_len]
             single_step_pred_qvel_orig = single_step_pred_qvel_orig[:test_seq_len]

        ground_truth_qpos_orig_for_test_seq = original_output_qpos[gt_align_start_idx:gt_align_end_idx, :]
        ground_truth_qvel_orig_for_test_seq = original_output_qvel[gt_align_start_idx:gt_align_end_idx, :]

    else: # Handle empty test predictions
        print("No single-step predictions to process.")
        single_step_pred_qpos_orig = np.array([]); single_step_pred_qvel_orig = np.array([])
        ground_truth_qpos_orig_for_test_seq = np.array([]); ground_truth_qvel_orig_for_test_seq = np.array([])


    # --- 11.5 Calculate Single Step Errors & Metrics ---
    # Moved calculation here to cover the *entire* test set predictions
    joint_names = ['Cab', 'Boom', 'Arm', 'Bucket'] # Assuming standard order
    if test_seq_len > 0 and len(single_step_pred_qpos_orig) == test_seq_len:
        print("\n--- Single-Step Prediction Metrics (Full Test Set) ---")
        # Calculate Errors
        single_step_qpos_error_full = np.zeros_like(single_step_pred_qpos_orig)
        single_step_qpos_error_full[:, cab_idx] = angle_difference(single_step_pred_qpos_orig[:, cab_idx], ground_truth_qpos_orig_for_test_seq[:, cab_idx])
        current_col = 0
        for idx in range(4):
            if idx != cab_idx:
                single_step_qpos_error_full[:, idx] = single_step_pred_qpos_orig[:, idx] - ground_truth_qpos_orig_for_test_seq[:, idx]
                # current_col += 1 # No need, indexing by absolute joint index idx

        single_step_qvel_error_full = single_step_pred_qvel_orig - ground_truth_qvel_orig_for_test_seq

        # MAE & RMSE
        ss_qpos_mae = np.mean(np.abs(single_step_qpos_error_full), axis=0)
        ss_qvel_mae = np.mean(np.abs(single_step_qvel_error_full), axis=0)
        ss_qpos_rmse = np.sqrt(np.mean(single_step_qpos_error_full**2, axis=0))
        ss_qvel_rmse = np.sqrt(np.mean(single_step_qvel_error_full**2, axis=0))

        print("MAE:")
        for i, name in enumerate(joint_names): print(f"  {name}: Pos={np.rad2deg(ss_qpos_mae[i]):.4f} deg, Vel={ss_qvel_mae[i]:.6f} rad/s")
        print("RMSE:")
        for i, name in enumerate(joint_names): print(f"  {name}: Pos={np.rad2deg(ss_qpos_rmse[i]):.4f} deg, Vel={ss_qvel_rmse[i]:.6f} rad/s")

        # Peak Error
        peak_qpos_error = np.max(np.abs(single_step_qpos_error_full), axis=0)
        peak_qvel_error = np.max(np.abs(single_step_qvel_error_full), axis=0)
        print("Peak Absolute Error:")
        for i, name in enumerate(joint_names): print(f"  {name}: Pos={np.rad2deg(peak_qpos_error[i]):.4f} deg, Vel={peak_qvel_error[i]:.6f} rad/s")

        # Velocity Direction Accuracy
        threshold = 1e-3 # Threshold for non-zero velocity
        correct_sign = np.zeros(4)
        non_zero_steps = np.zeros(4)
        for j in range(4):
            valid_steps_mask = np.abs(ground_truth_qvel_orig_for_test_seq[:, j]) > threshold
            non_zero_steps[j] = np.sum(valid_steps_mask)
            if non_zero_steps[j] > 0:
                sign_true = np.sign(ground_truth_qvel_orig_for_test_seq[valid_steps_mask, j])
                sign_pred = np.sign(single_step_pred_qvel_orig[valid_steps_mask, j])
                correct_sign[j] = np.sum(sign_true == sign_pred)
        direction_accuracy = np.zeros(4)
        # Calculate accuracy only where non_zero_steps > 0 to avoid division by zero
        mask_nz = non_zero_steps > 0
        direction_accuracy[mask_nz] = (correct_sign[mask_nz] / non_zero_steps[mask_nz]) * 100

        print("Velocity Direction Accuracy (% of non-zero steps):")
        for i, name in enumerate(joint_names): print(f"  {name}: {direction_accuracy[i]:.2f}% ({int(correct_sign[i])}/{int(non_zero_steps[i])} steps)")
        ...
        for j in range(4):
            # Qpos (handle potential NaNs if predictions were bad)
            mask_qpos = ~np.isnan(single_step_pred_qpos_orig[:, j]) & ~np.isnan(ground_truth_qpos_orig_for_test_seq[:, j])
            if np.sum(mask_qpos) > 1: # Need at least 2 points for correlation/r2
                 corr_qpos, _ = pearsonr(ground_truth_qpos_orig_for_test_seq[mask_qpos, j], single_step_pred_qpos_orig[mask_qpos, j])
                 try: r2_qpos = r2_score(ground_truth_qpos_orig_for_test_seq[mask_qpos, j], single_step_pred_qpos_orig[mask_qpos, j])
                 except ValueError: r2_qpos = np.nan # Handle cases where r2_score fails
            else: corr_qpos, r2_qpos = np.nan, np.nan
            correlations_qpos.append(corr_qpos)
            r2_scores_qpos.append(r2_qpos)

            # Qvel (handle potential NaNs)
            mask_qvel = ~np.isnan(single_step_pred_qvel_orig[:, j]) & ~np.isnan(ground_truth_qvel_orig_for_test_seq[:, j])
            if np.sum(mask_qvel) > 1:
                 corr_qvel, _ = pearsonr(ground_truth_qvel_orig_for_test_seq[mask_qvel, j], single_step_pred_qvel_orig[mask_qvel, j])
            ...
            correlations_qvel.append(corr_qvel)
            r2_scores_qvel.append(r2_qvel)

            print(f"  {joint_names[j]}: Pos R={corr_qpos:.4f}, R2={r2_qpos:.4f} | Vel R={corr_qvel:.4f}, R2={r2_qvel:.4f}")
    else:
        print("Skipping single-step metric calculation due to missing predictions or ground truth.")


    # --- 12. Multi-step Rollout Prediction ---
    # ... (同上一版本, 包含可选的 clip_rollout_input) ...
    print("\nPerforming multi-step rollout on a test segment...")
    start_index_in_test_seq = 0
    if X_test_seq is None or len(X_test_seq) <= start_index_in_test_seq:
        print("Not enough test sequences for rollout evaluation.")
        predictions_rollout_pos = np.array([]); predictions_rollout_vel = np.array([]); ground_truth_rollout_pos = np.array([]); ground_truth_rollout_vel = np.array([]); rollout_steps = 0
    else:
    ...
    if rollout_steps > 0 and len(single_step_pred_qpos_orig) >= rollout_steps:
       ...
       if rollout_steps > 0 and len(single_step_pred_qpos_segment)==0:
           print("Warning: Could not extract single step segment, clearing rollout results for plotting.")
           predictions_rollout_pos = np.array([]); predictions_rollout_vel = np.array([]); ground_truth_rollout_pos = np.array([]); ground_truth_rollout_vel = np.array([])


    # --- 14. Plotting Comparison (Rollout vs Single Step) ---
    # ... (同上一版本, 使用 dpi=300) ...
    ...
    if (len(predictions_rollout_pos) > 0 and len(single_step_pred_qpos_segment) > 0 and len(ground_truth_rollout_pos) > 0 and len(time_test_seq_end) >= len(ground_truth_rollout_pos)):
        ...
        # joint_names defined in section 11.5
        for i in range(num_joints_to_plot):
            axes[i, 0].plot(time_axis, np.rad2deg(ground_truth_rollout_pos[:plot_len, i]), label='Ground Truth', color='blue', linewidth=1.5, alpha=0.8)
            axes[i, 0].plot(time_axis, np.rad2deg(predictions_rollout_pos[:plot_len, i]), label='Rollout Pred', color='red', linestyle='--', linewidth=1, alpha=0.8)
            axes[i, 0].plot(time_axis, np.rad2deg(single_step_pred_qpos_segment[:plot_len, i]), label='Single-Step Pred', color='green', linestyle=':', linewidth=1, alpha=0.8)
            axes[i, 0].set_ylabel(f'{joint_names[i]} Pos (deg)'); axes[i, 0].legend(); axes[i, 0].grid(True)
            axes[i, 1].plot(time_axis, ground_truth_rollout_vel[:plot_len, i], label='Ground Truth', color='blue', linewidth=1.5, alpha=0.8)
            axes[i, 1].plot(time_axis, predictions_rollout_vel[:plot_len, i], label='Rollout Pred', color='red', linestyle='--', linewidth=1, alpha=0.8)
            axes[i, 1].plot(time_axis, single_step_pred_vel_segment[:plot_len, i], label='Single-Step Pred', color='green', linestyle=':', linewidth=1, alpha=0.8)
    ...
    if (len(predictions_rollout_pos) > 0 and len(ground_truth_rollout_pos) > 0 and len(predictions_rollout_pos) == len(ground_truth_rollout_pos)):
        ...
        # Rollout Error Plot
        # Ensure time axis matches error array length
        plot_len_err = min(len(rollout_qpos_error), len(time_test_seq_end) - start_index_in_test_seq)
        if plot_len_err > 0:
            time_axis_err = time_test_seq_end[start_index_in_test_seq : start_index_in_test_seq + plot_len_err]
            time_axis_err = time_axis_err - time_axis_err[0]
            ...
            for i in range(num_joints_to_plot):
                 axes_err[i, 0].plot(time_axis_err, np.rad2deg(rollout_qpos_error[:plot_len_err, i]), label=f'{joint_names[i]} Pos Error', color='green', linewidth=1); axes_err[i, 0].axhline(0, color='gray', linestyle='--', linewidth=0.8); axes_err[i, 0].set_ylabel('Error (deg)'); axes_err[i, 0].legend(); axes_err[i, 0].grid(True)
                 axes_err[i, 1].plot(time_axis_err, rollout_qvel_error[:plot_len_err, i], label=f'{joint_names[i]} Vel Error', color='purple', linewidth=1); axes_err[i, 1].axhline(0, color='gray', linestyle='--', linewidth=0.8); axes_err[i, 1].set_ylabel('Error (rad/s)'); axes_err[i, 1].legend(); axes_err[i, 1].grid(True)
        ...
        else: print("Skipping rollout error plot due to insufficient data.")
    ...
    if test_seq_len > 0 and len(single_step_qpos_error_full) == test_seq_len:
        ...
        fig_ss_err, axes_ss_err = plt.subplots(num_joints_to_plot, 2, figsize=(18, 3.5 * num_joints_to_plot), sharex=True)
        ...
        # Time axis for the full single-step test sequence
        time_axis_full_test = time_test_seq_end[:test_seq_len] # Use the aligned time
        time_axis_full_test = time_axis_full_test - time_axis_full_test[0] # Start at 0

        for i in range(num_joints_to_plot):
            # Position Error
            axes_ss_err[i, 0].plot(time_axis_full_test, np.rad2deg(single_step_qpos_error_full[:, i]), label=f'{joint_names[i]} Pos Error', color='orange', linewidth=1)
            axes_ss_err[i, 0].axhline(0, color='gray', linestyle='--', linewidth=0.8)
            axes_ss_err[i, 0].set_ylabel('Error (deg)')
            axes_ss_err[i, 0].legend()
            axes_ss_err[i, 0].grid(True)
            # Velocity Error
            axes_ss_err[i, 1].plot(time_axis_full_test, single_step_qvel_error_full[:, i], label=f'{joint_names[i]} Vel Error', color='cyan', linewidth=1)
            axes_ss_err[i, 1].axhline(0, color='gray', linestyle='--', linewidth=0.8)
            axes_ss_err[i, 1].set_ylabel('Error (rad/s)')
            axes_ss_err[i, 1].legend()
            axes_ss_err[i, 1].grid(True)

        axes_ss_err[-1, 0].set_xlabel('Time (s)'); axes_ss_err[-1, 1].set_xlabel('Time (s)')
        plt.tight_layout(rect=[0, 0.03, 1, 0.96])
        plt.savefig(single_step_error_plot_save_path, dpi=300); # High DPI
        plt.close(fig_ss_err)
        print(f"Single-step error plot saved to {single_step_error_plot_save_path}")
    else:
        print("Skipping single-step error plotting due to missing data.")
